...
const INSIGHT_STYLES: Record<string, { icon: typeof CheckCircle2; color: string; bg: string; border: string }> = {
  ...
  info: { icon: Info, color: 'text-[#6B7280]', bg: 'bg-black/[0.02]', border: 'border-black/[0.08]' },
}
      ...
      {segments.filter(s => s.value > 0).map((seg, i) => (
        ...
        >
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <span className="text-[10px] font-bold text-[#111827] drop-shadow-sm whitespace-nowrap">
              {seg.label}
            </span>
          </div>
        </div>
...
export default function OccupancyIntelligence({ facilityId, adminKey }: {
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-black/[0.08] bg-white p-12 text-center">
        ...
        <p className="text-[#6B7280]">Loading occupancy intelligence...</p>
      </div>
    )
  }
  ...
  if (!data || data.unit_occupancy.length === 0) {
    return (
      <div className="rounded-xl border border-black/[0.08] bg-white p-12 text-center">
        <Building2 className="mx-auto mb-3 text-[#9CA3AF]" size={40} />
      ...
      <div className="rounded-xl border border-black/[0.08] bg-white p-5">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-6 mt-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-[#6B7280]">Physical {pct(fl.physical_occ_pct)}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-gradient-to-r from-indigo-500 to-indigo-400" />
                <span className="text-xs font-medium text-[#6B7280]">Economic {pct(fl.economic_occ_pct)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 content-center">
            <div className="rounded-lg p-3 bg-black/[0.02]">
              <p className="text-xs font-medium text-[#9CA3AF]">Total Units</p>
              <p className="text-xl font-bold text-[#111827]">{fl.total_units}</p>
              <p className="text-xs text-[#9CA3AF]">{fl.occupied_units} occupied</p>
            </div>
            <div className="rounded-lg p-3 bg-black/[0.02]">
              <p className="text-xs font-medium text-[#9CA3AF]">Vacant</p>
              <p className="text-xl font-bold text-red-500">{fl.vacant_units}</p>
              <p className="text-xs text-[#9CA3AF]">{money(gd.vacancy_loss)}/mo lost</p>
            </div>
            <div className="rounded-lg p-3 bg-black/[0.02]">
              <p className="text-xs font-medium text-[#9CA3AF]">Gross Potential</p>
              <p className="text-xl font-bold text-[#111827]">{money(fl.gross_potential)}</p>
              <p className="text-xs text-[#9CA3AF]">at street rates</p>
            </div>
            <div className="rounded-lg p-3 bg-black/[0.02]">
              <p className="text-xs font-medium text-[#9CA3AF]">Actual MRR</p>
              <p className="text-xl font-bold text-emerald-500">{money(fl.actual_revenue)}</p>
              <p className="text-xs text-[#9CA3AF]">{pct(fl.economic_occ_pct)} capture</p>
            </div>
          </div>

          {/* Multi-dimensional occupancy */}
          <div className="space-y-4 flex flex-col justify-center">
            <h4 className="text-sm font-semibold text-[#111827]">Multi-Dimensional Occupancy</h4>
            {[
              ...
              <div key={bar.label}>
                <div className="flex justify-between mb-1">
                  ...
                  <span className="text-xs font-bold text-[#111827]">{pct(bar.value)}</span>
                </div>
                <div className="w-full h-4 rounded-full bg-black/[0.03]">
                  <div
      ...
      {insights.length > 0 && (
        <div className="rounded-xl border border-black/[0.08] bg-white">
          ...
          {expandedSection === 'insights' && (
            <div className="border-t border-black/[0.08] p-4 space-y-3">
              {insights.map((ins, i) => {
                ...
                return (
                  <div key={i} className={`rounded-lg p-4 border ${style.bg} ${style.border}`}>
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-[#111827]">{ins.title}</p>
                        <p className="text-xs mt-1 text-[#6B7280]">{ins.detail}</p>
                      </div>
                    </div>
                  </div>
                )
      ...
      <div className="rounded-xl border border-black/[0.08] bg-white">
        ...
        {expandedSection === 'gap' && (
          <div className="border-t border-black/[0.08] p-4 space-y-5">
            {/* Visual waterfall */}
            <div>
              <p className="text-xs font-medium uppercase tracking-wide text-[#9CA3AF] mb-3">Revenue Drag Breakdown</p>
              <StackedBar
                height={36}
                segments={[
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              <div className="rounded-lg p-4 border border-red-500/20 bg-red-500/5">
                ...
                <p className="text-xs text-[#9CA3AF]">/mo &middot; {pct(gd.vacancy_drag_pct)} of potential</p>
                <p className="text-xs mt-2 text-[#9CA3AF]">{fl.vacant_units} empty units at street rate</p>
              </div>
              ...
              <div className="rounded-lg p-4 border border-amber-500/20 bg-amber-500/5">
                ...
                <p className="text-xs text-[#9CA3AF]">/mo &middot; {pct(gd.rate_gap_drag_pct)} of potential</p>
              ...
              <div className="rounded-lg p-4 border border-orange-500/20 bg-orange-500/5">
                ...
                <p className="text-xs text-[#9CA3AF]">/mo &middot; {pct(gd.discount_drag_pct)} of potential</p>
                <p className="text-xs mt-2 text-[#9CA3AF]">{discTenants.length} tenants with active discounts</p>
              </div>
              ...
              <div className="rounded-lg p-4 border border-purple-500/20 bg-purple-500/5">
                ...
                <p className="text-xs text-[#9CA3AF]">/mo &middot; {pct(gd.delinquency_drag_pct)} of potential</p>
                <p className="text-xs mt-2 text-[#9CA3AF]">{ab.count} delinquent accounts</p>
              </div>
            </div>

            {/* Revenue bridge */}
            <div className="rounded-lg p-4 bg-black/[0.02]">
              <p className="text-xs font-medium uppercase tracking-wide text-[#9CA3AF] mb-3">Revenue Bridge</p>
              <div className="flex items-center gap-2 flex-wrap text-sm">
                <span className="font-bold text-[#111827]">{money(fl.gross_potential)}</span>
                <span className="text-[#9CA3AF]">Potential</span>
                <ArrowRight size={14} className="text-[#9CA3AF]" />
                <span className="font-bold text-red-500">-{money(gd.vacancy_loss)}</span>
                <span className="text-[#9CA3AF]">Vacancy</span>
                <ArrowRight size={14} className="text-[#9CA3AF]" />
                <span className="font-bold text-amber-500">-{money(gd.rate_gap_loss)}</span>
                <span className="text-[#9CA3AF]">Rate Gap</span>
                <ArrowRight size={14} className="text-[#9CA3AF]" />
                <span className="font-bold text-orange-500">-{money(gd.discount_drag)}</span>
                <span className="text-[#9CA3AF]">Discounts</span>
                <ArrowRight size={14} className="text-[#9CA3AF]" />
                <span className="font-bold text-purple-500">-{money(gd.delinquency_drag)}</span>
                <span className="text-[#9CA3AF]">Delinquency</span>
                <ArrowRight size={14} className="text-[#9CA3AF]" />
                <span className="font-bold text-emerald-500">{money(fl.actual_revenue)}</span>
                <span className="text-[#9CA3AF]">Collected</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="border-t border-black/[0.08]">
            ...
            <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {unitOcc.map((u, i) => {
                ...
                return (
                  <div key={i} className="rounded-lg p-4 border border-black/[0.08] bg-black/[0.02]">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        <p className="text-sm font-semibold text-[#111827]">{u.unit_type}</p>
                        <p className="text-xs text-[#9CA3AF]">{u.sqft} sqft &middot; {u.total_count} units</p>
                      </div>
                    ...
                    <div className="space-y-2">
                      <div>
                        <div className="flex justify-between mb-0.5">
                          <span className="text-[10px] font-medium text-[#9CA3AF]">Physical</span>
                          <span className="text-[10px] font-bold text-[#111827]">{pct(u.physical_occ_pct)}</span>
                        </div>
                        <div className="w-full h-2.5 rounded-full bg-black/[0.03]">
                      ...
                      <div>
                        <div className="flex justify-between mb-0.5">
                          <span className="text-[10px] font-medium text-[#9CA3AF]">Economic</span>
                          <span className="text-[10px] font-bold text-[#111827]">{pct(u.economic_occ_pct)}</span>
                        </div>
                        <div className="w-full h-2.5 rounded-full bg-black/[0.03]">
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-dashed border-black/[0.08]">
                      <div className="text-center">
                        <p className="text-[10px] text-[#9CA3AF]">Gap</p>
                        <p className={`text-sm font-bold ${gapColor}`}>{u.gap > 0 ? '+' : ''}{u.gap}pt</p>
                      </div>
                      <div className="text-center">
                        <p className="text-[10px] text-[#9CA3AF]">Vacancy $</p>
                        <p className="text-sm font-bold text-red-500">{money(u.vacancy_cost_monthly)}</p>
                      </div>
                      <div className="text-center">
                        <p className="text-[10px] text-[#9CA3AF]">Rate Gap $</p>
            ...
            {/* Detailed table */}
            <div className="border-t border-black/[0.08] overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-black/[0.02]">
                    {['Unit Type', 'Units', 'Physical', 'Economic', 'Gap', 'Street', 'Avg Actual', '$/sqft', 'Total Drag'].map((h, i) => (
                      <th key={i} className={`${i === 0 ? 'text-left px-4' : i < 5 ? 'text-center px-3' : 'text-right px-3'} py-3 font-medium text-[#9CA3AF] ${i === 8 ? 'pr-4' : ''}`}>{h}</th>
                    ))}
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={i} className="hover:bg-black/[0.03]">
                        <td className="px-4 py-3 font-medium text-[#111827]">
                          <div>{u.unit_type}</div>
                          <div className="text-xs text-[#9CA3AF]">{u.sqft} sqft</div>
                        </td>
                        <td className="px-3 py-3 text-center">
                          <span className="text-[#111827]">{u.occupied_count}</span>
                          <span className="text-[#9CA3AF]">/{u.total_count}</span>
                        ...
                        <td className="px-3 py-3 text-right font-mono text-[#111827]">${u.street_rate?.toFixed(0)}</td>
                        ...
                        <td className="px-3 py-3 text-right font-mono text-[#111827]">
                ...
                <tfoot>
                  <tr className="bg-black/[0.02] border-t border-black/[0.1]">
                    <td className="px-4 py-3 font-bold text-[#111827]">FACILITY</td>
                    <td className="px-3 py-3 text-center">
                      <span className="font-bold text-[#111827]">{fl.occupied_units}</span>
                      <span className="text-[#9CA3AF]">/{fl.total_units}</span>
                    </td>
                    ...
                    <td className="px-3 py-3 text-right font-bold text-[#111827]">{money(fl.gross_potential)}</td>
                    <td className="px-3 py-3 text-right font-bold text-emerald-500">{money(fl.actual_revenue)}</td>
                    <td className="px-3 py-3 text-right text-[#9CA3AF]">--</td>
        ...
        <div className="rounded-xl border border-black/[0.08] bg-white">
          ...
          {expandedSection === 'trend' && (
            <div className="border-t border-black/[0.08] p-4 space-y-4">
              {/* Revenue sparkline */}
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-[#9CA3AF] mb-2">Monthly Revenue</p>
                  ...
                  {trend.map((t, i) => {
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
                        <div
                          className={`w-full rounded-t transition-all ${isLatest ? 'bg-emerald-500' : 'bg-black/[0.06] group-hover:bg-emerald-500/40'}`}
                          style={{ height: `${Math.max(h, 2)}%` }}
                        />
                        <span className="text-[8px] text-[#9CA3AF] leading-none">{t.month_short}</span>
                        <div className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 hidden group-hover:block z-10">
                          <div className="rounded-lg p-2 text-xs shadow-lg whitespace-nowrap bg-[#F3F4F6] text-[#111827] border border-black/[0.08]">
                    ...
                    return (
                      <div key={i} className="flex-1 flex flex-col items-center relative" style={{ height: '100%' }}>
                        ...
                        <div className="absolute bottom-0">
                          <span className="text-[8px] text-[#9CA3AF]">{t.month_short}</span>
                        </div>
                      </div>
                    )
                  })}
        ...
        <div className="rounded-xl border border-black/[0.08] bg-white">
          ...
          {expandedSection === 'delinquency' && (
            <div className="border-t border-black/[0.08] p-4 space-y-4">
              {/* Aging buckets */}
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-[#9CA3AF] mb-3">Aging Distribution</p>
                <StackedBar
                  height={32}
                  segments={[
                ...
                <div className="flex gap-3 mt-2 flex-wrap">
                  {[
                    ...
                    <div key={b.label} className="flex items-center gap-1.5">
                      ...
                      <span className="text-xs text-[#9CA3AF]">{b.label}: <span className="font-medium text-[#111827]">{money(b.value)}</span></span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {delTenants.length > 0 && (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-black/[0.02]">
                        <th className="text-left px-4 py-2 font-medium text-[#9CA3AF]">Unit</th>
                        <th className="text-left px-3 py-2 font-medium text-[#9CA3AF]">Tenant</th>
                        <th className="text-right px-3 py-2 font-medium text-[#9CA3AF]">Rent</th>
                        <th className="text-right px-3 py-2 font-medium text-[#9CA3AF]">Owed</th>
                        <th className="text-center px-3 py-2 font-medium text-[#9CA3AF]">Days Past Due</th>
                        <th className="text-center px-4 py-2 font-medium text-[#9CA3AF]">Risk</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-black/[0.06]">
                      {delTenants.slice(0, 20).map((t, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-black/[0.03]">
                            ...
                            <td className="px-3 py-2 text-[#111827]">{t.tenant}</td>
                            <td className="px-3 py-2 text-right font-mono text-[#111827]">{money(t.rent_rate)}</td>
                  ...
                  {delTenants.length > 20 && (
                    <p className="text-xs text-center py-2 text-[#9CA3AF]">
          ...
          {expandedSection === 'discounts' && (
            <div className="border-t border-black/[0.08] overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-black/[0.02]">
                    <th className="text-left px-4 py-3 font-medium text-[#9CA3AF]">Unit</th>
                    <th className="text-left px-3 py-3 font-medium text-[#9CA3AF]">Tenant</th>
                    <th className="text-left px-3 py-3 font-medium text-[#9CA3AF]">Type</th>
                    <th className="text-right px-3 py-3 font-medium text-[#9CA3AF]">Standard</th>
                    <th className="text-right px-3 py-3 font-medium text-[#9CA3AF]">Actual</th>
                    <th className="text-right px-3 py-3 font-medium text-[#9CA3AF]">Discount</th>
                    <th className="text-left px-3 py-3 font-medium text-[#9CA3AF]">Reason</th>
                    <th className="text-right px-4 py-3 font-medium text-[#9CA3AF]">Tenure</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-black/[0.06]">
                  {discTenants.map((t, i) => (
                    <tr key={i} className="hover:bg-black/[0.03]">
                      <td className="px-4 py-2.5 font-medium text-[#111827]">{t.unit}</td>
                      <td className="px-3 py-2.5 text-[#111827]">{t.tenant}</td>
                      <td className="px-3 py-2.5 text-[#6B7280]">{t.unit_type}</td>
                      <td className="px-3 py-2.5 text-right font-mono text-[#111827]">${t.standard_rate.toFixed(0)}</td>
                      ...
                      <td className="px-3 py-2.5 text-xs text-[#9CA3AF]">{t.discount_desc || '--'}</td>
                      <td className="px-4 py-2.5 text-right text-[#9CA3AF]">{Math.round(t.days_as_tenant / 30)}mo</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-black/[0.02] border-t border-black/[0.1]">
                    <td colSpan={5} className="px-4 py-2.5 font-bold text-[#111827]">
                    ...
                    <td colSpan={2} className="px-3 py-2.5 text-right text-xs text-[#9CA3AF]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="rounded-lg p-4 border border-blue-500/20 bg-blue-500/5">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm font-semibold text-[#111827]">Slight ($1-10 below)</span>
                  </div>
                  <p className="text-3xl font-bold text-blue-500">{bands.slight.count}</p>
                  <p className="text-xs text-[#9CA3AF]">tenants &middot; {money(bands.slight.loss)}/mo</p>
                  <p className="text-xs mt-2 text-[#9CA3AF]">Low priority \u2014 monitor at next lease renewal</p>
                </div>
                <div className="rounded-lg p-4 border border-amber-500/20 bg-amber-500/5">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-3 h-3 rounded-full bg-amber-400" />
                    <span className="text-sm font-semibold text-[#111827]">Moderate ($11-30 below)</span>
                  </div>
                  <p className="text-3xl font-bold text-amber-500">{bands.moderate.count}</p>
                  <p className="text-xs text-[#9CA3AF]">tenants &middot; {money(bands.moderate.loss)}/mo</p>
                  <p className="text-xs mt-2 text-[#9CA3AF]">ECRI candidates \u2014 phase increases over 2-3 months</p>
                </div>
                <div className="rounded-lg p-4 border border-red-500/20 bg-red-500/5">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-3 h-3 rounded-full bg-red-500" />
                    <span className="text-sm font-semibold text-[#111827]">Severe ($30+ below)</span>
                  </div>
                  <p className="text-3xl font-bold text-red-500">{bands.severe.count}</p>
                  <p className="text-xs text-[#9CA3AF]">tenants &middot; {money(bands.severe.loss)}/mo</p>
                  <p className="text-xs mt-2 text-[#9CA3AF]">Immediate action \u2014 largest revenue recovery opportunity</p>
                </div>
              </div>
            </div>
          )}
