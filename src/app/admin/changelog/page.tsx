...
/* ═══════════════════════════════════════════
   Rich changelog — admin-only developer commit log. Syncs commits from
   GitHub, classifies them, and renders AI-generated summaries with author,
   hash, and GitHub links. Backed by /api/commit-notes (+ /sync) and the
   commit_enrichments table. The manually-curated partner announcements live
   at /admin/changelog/announcements.
   ═══════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════
   CONSTANTS — category colors follow the light-only design system:
   green / blue categorical, neutral charcoal for fixes (red is errors-only;
   gold is banned outside the logo lockup).
   ═══════════════════════════════════════════ */
  ...
  return (
    <div ref={dropdownRef} className="relative">
      <button
        ...
        style={{
          backgroundColor: selected ? "var(--color-dark)" : "var(--color-light-gray)",
      ...
      {open && (
        ...
        >
          <button
            ...
            style={{
              color: !selected ? "var(--color-dark)" : "var(--color-body-text)",
          ...
          {developers.map((dev) => (
            <button
              ...
              className="w-full text-left px-3 py-2 text-xs transition-colors hover:bg-[var(--color-light-gray)]"
              style={{
                color: selected === dev ? "var(--color-dark)" : "var(--color-body-text)",
...
function SyncBanner({ result, onDismiss }: { result: string; onDismiss: () => void }) {
  const isError =
        ...
        <div className="flex-1 min-w-0">
          ...
          >
            ...
            <a
              ...
              className="inline-flex items-center gap-0.5 transition-colors hover:text-[var(--color-dark)]"
...
export default function ChangelogPage() {
  ...
  // ── Fetch data ──
  const { data, loading, error, refetch } = useAdminFetch<ChangelogResponse>(
    "/api/commit-notes",
    params,
  );
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs sm:text-sm mt-1" style={{ color: "var(--color-mid-gray)" }}>
            {totalCount > 0
              ? `${totalCount} commit${totalCount === 1 ? "" : "s"} tracked`
              : "Developer commit log, synced from GitHub"}
          </p>
        </div>

        <div className="flex items-center gap-2 self-start sm:self-auto">
          <Link
            href="/admin/changelog/announcements"
          ...
          <button
            ...
            style={{
              backgroundColor: syncing ? "var(--color-light-gray)" : "var(--color-dark)",
              color: syncing ? "var(--color-mid-gray)" : "var(--color-light)",
          ...
          >
            ...
            <span className="hidden sm:inline">{syncing ? "Syncing..." : "Sync Commits"}</span>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        >
          ...
          {TYPE_FILTERS.map((f) => {
            ...
            const count = typeCounts?.[f.key as keyof typeof typeCounts] ?? null;
            return (
              <button
                key={f.key}
                type="button"
      ...
      {isInitialLoad ? (
        ...
        <div className="space-y-8">
          ...
          {!hasMore && allEntries.length > 0 && (
            <p className="text-center text-xs py-4" style={{ color: "var(--color-mid-gray)" }}>
        ...
        <div className="text-center py-16 sm:py-24">
          ...
          {hasActiveFilters ? (
            ...
            <>
              ...
              >
                Sync your Git history to populate the changelog with AI-generated summaries of
                every commit
              </p>
              <button
                type="button"
