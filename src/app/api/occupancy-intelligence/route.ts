...
export async function GET(request: NextRequest) {
  ...
  try {
    const [unitsRaw, snapshotsRaw, tenantRates, agingRows, revenueHistory, rentRoll] =
      await Promise.all([
        queryUnitsWithOccupancy(facilityId),
        querySnapshots(facilityId, 90),
        queryTenantRatesDetailed(facilityId),
        queryAgingLatest(facilityId),
        queryRevenueHistoryAsc(facilityId),
        queryRentRollLatest(facilityId),
      ]);
