...
**Shipped:** `client_messages` table; `/api/client-messages` rewritten onto Postgres (wire contract unchanged, reading a thread marks the other party's messages read); admin inbox at `/admin/messages` + `/api/admin-message-threads` (thread list, unread counts) with a REVENUE nav entry. **Notification-on-new-message is fully wired in both directions (commit 45cfc06):** push (already in place) AND email via `src/lib/email.ts` — an admin reply emails the client (reply-to founder inbox), a client message emails `ADMIN_EMAIL` (reply-to the client); both fire-and-forget, idempotency-keyed on the message id. Email is the channel that actually lands in alpha (push needs an installed PWA). **Requires the prod DDL apply** (see end of doc).
